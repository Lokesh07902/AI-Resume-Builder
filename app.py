            ...
            if st.button("✨  Generate Resume",disabled=not req,use_container_width=True):
                with st.spinner("🤖 Gemini AI is crafting your resume..."):
                    res=call_gemini(build_resume_prompt({
                        ...
                        "skills":st.session_state.skills}),temp,topp)
                    st.session_state.resume_md=res
                    st.session_state.resume_done=True
                    st.session_state.chat=[{"role":"assistant","content":"✅ Resume ready! Use the tabs above to Score, Tailor, get Cover Letter, Interview Questions and more!"}]
                st.success("✅ Resume generated! Now explore all the tabs ↑")
...
<div style="font-size:.78rem;color:#091820;margin-top:.3rem">Fill your details and click Generate</div>
    ...
    with tab2:
        ...
        if not st.session_state.resume_done:
            st.info("👆 Generate your resume first in the Build tab.")
        else:
    ...
    with tab3:
        ...
        if not st.session_state.resume_done:
            st.info("👆 Generate your resume first.")
        else:
            st.markdown('<div class="icard">🎯 AI analyzes your resume against ATS criteria and gives a score out of 100 with specific improvement tips.</div>',unsafe_allow_html=True)
            if st.button("🔍  Analyze & Score My Resume",use_container_width=True):
...
box-shadow:0 10px 40px rgba(0,0,0,.4)">{rhtml}</div>""",unsafe_allow_html=True)
    ...
    with tab5:
        ...
        if not st.session_state.resume_done:
            ...
            st.markdown('<div class="icard">✉️ AI generates a personalized, professional cover letter based on your resume.</div>',unsafe_allow_html=True)
            ...
            if st.button("📝  Generate Cover Letter",use_container_width=True):
                with st.spinner("🤖 Writing your cover letter..."):
                    prompt=f"""Write a professional cover letter based on this resume.
RESUME:\n{st.session_state.resume_md}
Company: {company or 'the company'}
Position: {position or st.session_state.job_title}
Tone: {tone}
Instructions:
- 3-4 paragraphs: opening, skills match, motivation, closing
...
Output ONLY the cover letter text."""
    ...
    with tab6:
        ...
        if not st.session_state.resume_done:
            ...
            st.markdown('<div class="icard">🎤 AI generates likely interview questions based on your resume + suggested answers.</div>',unsafe_allow_html=True)
            ...
            if st.button("🎤  Generate Interview Questions",use_container_width=True):
                with st.spinner("🤖 Generating interview questions..."):
                    prompt=f"""Based on this resume, generate {num_q} likely interview questions with suggested answers.
RESUME:\n{st.session_state.resume_md}
Type: {q_type}
Format each as:
Q[number]: [Question]
...
Be specific to the candidate's experience."""
    ...
    with tab7:
        ...
        if not st.session_state.resume_done:
            ...
            st.markdown('<div class="icard">📊 Enter your dream job title → AI compares your current skills and tells you exactly what\'s missing.</div>',unsafe_allow_html=True)
            dream_job=st.text_input("Enter Your Dream Job Title",placeholder="Data Scientist, Full Stack Developer, DevOps Engineer...")
            if st.button("🔍  Analyze Skill Gap",disabled=not dream_job.strip(),use_container_width=True):
                with st.spinner("🤖 Analyzing skill gaps..."):
                    prompt=f"""Analyze the skill gap between this candidate's current skills and what's required for: {dream_job}
RESUME:\n{st.session_state.resume_md}
Provide:
1. CURRENT SKILLS MATCH: Skills they already have that are relevant (with ✅)
2. MISSING CRITICAL SKILLS: Must-have skills they lack (with ❌)
3. NICE-TO-HAVE SKILLS: Additional skills that would help (with 💡)
...
Be specific and encouraging."""
    ...
    with tab8:
        ...
        if not st.session_state.resume_done:
            ...
            st.markdown('<div class="icard">🔗 AI generates an optimized LinkedIn About section from your resume to attract recruiters.</div>',unsafe_allow_html=True)
            ...
            if st.button("💼  Generate LinkedIn Summary",use_container_width=True):
                with st.spinner("🤖 Crafting your LinkedIn summary..."):
                    prompt=f"""Write an optimized LinkedIn 'About' section based on this resume.
RESUME:\n{st.session_state.resume_md}
Tone: {li_tone}
Length: {li_len}
Instructions:
...
- Include top skills and expertise
